...
sens_Catm_Sobol$S
# useful to add names
row.names(sens_Catm_Sobol$S) <- colnames(parms) # add row names back in 
sens_Catm_Sobol$S # most important param is likely k_d

# total effect - accounts for parameter interactions
row.names(sens_Catm_Sobol$T) <- colnames(parms) # add row names back in 
...
- in order to run these sobel indices in your output needs to be summarized by one metric 
- If your output pops out a time series, you need to summarize it in some way to run this sensitivity analysis 
...
# NOTE: you can see height really isnt making a difference 
...
# NOTE: k_d most important parameter, very evident in this plot 
# Best to put most important param on the x-axis, and color by a different parameter 


# use second most sensitive parameter (using most important as color)
ggplot(both, aes(k_o, gs, col = k_d)) +
  geom_point() +
  labs(y = "Conductance (mm/s)", x = "k_o parameter")
```
...
sens_Catm_Sobol2$S2
# NOTES: if you cross zero between min. ci and max. ci, it means there is not a significant effect 
```
