...
-   what issue jumps out - what might be the cause?
    -   The value for the last year, 1954, is a bit lower than the other years
    -   After looking at the data, it is revealed that the 1954 data stops at September
-   how would you fix this?
    -   Can just remove that year, or if for some reason you needed the year
...
```{r}
# Remove 1954 from data 
sierraczosolar <- sierraczosolar %>%
  filter(year < 1954)

# Check that 1954 is no longer in data 
tail(sierraczosolar)
```
...
# generate possible values for efficiency  
